...
**The dictionary was fitted to the wrong corpus.** The shipped build is sized for browser and web content; measured against a 200-transcript corpus, **88% of its entries never fire** — a third of its capacity holds HTML structure and CSS utility classes. It was not short of vocabulary, it was carrying the wrong vocabulary. So we built one from 22 public-domain books, held four out, and trained one zstd dictionary on exactly the same 22 books. On the held-out set, the Elo stack now comes in **0.8% ahead** of the best zstd configuration. On four books that is a tie — but it is a large move from 25% behind, and the only thing that changed was which corpus the dictionary was built from.

**And the resident cost was measured on the wrong artifact.** We had been quoting the *runtime* database — a B-tree carrying page overhead and a reverse map derivable from the forward one. None of that needs to travel. Serialised and compressed, the same content is **19.4x smaller**, putting it *below* the size of the zstd dictionary it was losing to. The browser's vocabulary shrinks 8x against the JSON it ships today, and loads in 9ms rather than 107ms.

Two things broke on the way. A build parameter that looked declared was hardcoded, so no build had ever actually been fitted to its own corpus — the first attempt came out with 81,434 phrases from the *wrong* corpus against 1,819 from the right one, topped by `you know` at 539,244 occurrences. Victorian novels do not say that half a million times. And our own compact format silently lost 33 entries, because newline-delimiting a list whose members contain newlines drops every one of them. That is the third time this week a newline in a delimiter position has quietly corrupted a format that looked fine.

A fourth thing broke after publication: this post originally explained the 20x with a tidy mechanism that turned out to be false, and we have corrected it. The measurements were real; the explanation was reached for afterwards to make them make sense, and it survived because it sounded like the sort of thing that would be true.
